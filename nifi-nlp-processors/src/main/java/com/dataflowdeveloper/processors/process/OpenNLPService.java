...
public class OpenNLPService {
	 ...
	 */
	public List<Organization> getOrganizations(String modelDirectory, String sentence) {
		if ( sentence == null ) {
			return null;
		}
		TokenNameFinderModel model = null;
		...
		try {
			tokenStream = new FileInputStream(new File(modelDirectory + CURRENT_TOKEN_FILE));
			model = new TokenNameFinderModel( new File(modelDirectory + CURRENT_ORGANIZATION_FILE) );
		...
		if ( model != null && sentence != null) {
			try {
				...
				String[] spanns = Span.spansToStrings(nameSpans, tokens);
				for (int i = 0; i < spanns.length; i++) {
					orgs.add(new Organization(spanns[i]));
				}
				finder.clearAdaptiveData();
			} catch (Exception e) {
				e.printStackTrace();
